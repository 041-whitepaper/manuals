...
3. リモートリポジトリをつくる
	...
	2. GitHubにリモートリポジトリを作る
		...
		9. コマンドが書いてある画面が出現します。そのコマンドは後で使うのでページを保持しておいてください。
4. GitHubでssh接続する
...
6. リモートリポジトリをクローンする
	1. ローカルに作業環境を構築する
		...
		3. 3.2.9.の画面に戻ります。
		4. `or create a new repository on the command line`と書いてあるコマンドを全てコピーします。
		5. 6.1.2.のgit bashで実行します。
		6. うまく動作すればおkです。
7. すでにある作業フォルダをリモートリポジトリにアップする
	1. 作業環境でgit bashを実行する
		1. エクスプローラーで開き、右クリックのコンテキストメニューから実行します。
			1. どこで実行されているかは黄色文字で場所が表示されるのでそれを確認します。
	2. フォルダをアップする
		1. `or push an existing repository from the command line`と書いてあるコマンドをすべてコピーします。
		2. 7.1.のgit bashで実行します。
		3. うまく動作すればおkです。
8. 確認する
